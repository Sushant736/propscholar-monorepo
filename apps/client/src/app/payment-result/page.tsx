...
function PaymentResultContent() {
    ...
    const verifyPayment = async () => {
      try {
        ...
        if (!finalOrderId) {
          setResult({
            status: 'ERROR',
            message: 'No order information found. Please contact support if you were charged.',
          });
          return;
        }

        // Verify payment status with backend
        const order = await ordersApi.getById(finalOrderId);
        ...
        } else if (order.paymentDetails?.status === 'failed' || order.paymentDetails?.status === 'cancelled') {
          setResult({
            status: 'FAILED',
            order,
  ...
  const getStatusIcon = () => {
    switch (result.status) {
      ...
      case 'LOADING':
        return <Loader2 className="h-16 w-16 text-prop-scholar-electric-blue animate-spin" />;
      default:
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-prop-scholar-main-text">
                        {result.order.paymentDetails?.paymentMethod || 'PhonePe'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {(result.order.paymentDetails?.status === 'failed' || result.order.paymentDetails?.status === 'cancelled') && (
                        <XCircle className="h-4 w-4 text-red-500" />
                      )}
                      {(result.order.paymentDetails?.status === 'pending' || result.order.paymentDetails?.status === 'processing') && (
                        <Clock className="h-4 w-4 text-yellow-500" />
                      )}
                      <span className="text-prop-scholar-main-text capitalize">
                        {result.order.paymentDetails?.status?.toLowerCase() || 'Processing'}
                      </span>
                    </div>
                  </div>
                  ...
                  {result.order.paymentDetails?.paymentTimestamp && (
                    <div>
                      ...
                      <p className="text-prop-scholar-main-text mt-1">
                        {formatDate(result.order.paymentDetails.paymentTimestamp)}
                      </p>
                    </div>
                  )}
                </div>
            ...
            >
              <div className="bg-orange-500/10 border border-orange-500/20 rounded-2xl p-6">
                ...
                <p className="text-prop-scholar-secondary-text mb-4">
                  If you were charged but don&apos;t see your order, or if you&apos;re experiencing issues, 
                  please contact our support team with your order details.
                </p>
                <div className="flex flex-col sm:flex-row gap-2 justify-center text-sm">
                  <span className="text-prop-scholar-secondary-text">Email: support@propscholar.com</span>
                  <span className="text-prop-scholar-secondary-text hidden sm:inline">|</span>
                  <span className="text-prop-scholar-secondary-text">Phone: +91 12345 67890</span>
                </div>
              </div>
            </motion.div>
          )}
